...
RUN set -x \
    ...
    && apt-get update && apt-get install -y --no-install-recommends dirmngr gnupg gnupg2 wget && rm -rf /var/lib/apt/lists/* \
    ...
    && gosu nobody true \
    && apt-get purge -y --auto-remove dirmngr gnupg gnupg2 wget
