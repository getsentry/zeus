...
def test_build_styleviolations_list(
    ...
    violation1 = factories.StyleViolationFactory(job=job1, filename="bar")
    violation2 = factories.StyleViolationFactory(job=job2, filename="foo")
    db_session.add(violation1)
    db_session.add(violation2)

    resp = client.get(
        "/api/repos/{}/builds/{}/style-violations".format(
...
def test_build_styleviolations_list_severity_filter(
    ...
    violation = factories.StyleViolationFactory(
        job=default_job, severity=Severity.error
    )

    resp = client.get(
        "/api/repos/{}/builds/{}/style-violations?severity=warning".format(
            default_repo.get_full_name(), default_build.number
        )
    )
    assert resp.status_code == 200
    ...
    resp = client.get(
        "/api/repos/{}/builds/{}/style-violations?severity=error".format(
            default_repo.get_full_name(), default_build.number
        )
    )
    assert resp.status_code == 200
