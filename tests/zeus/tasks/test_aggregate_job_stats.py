...
def test_unfinished_job(mocker, db_session, default_project, default_source):
    ...
    build = factories.BuildFactory(project=default_project, source=default_source, queued=True)
...
def test_finished_job(mocker, db_session, default_project, default_source):
    ...
    build = factories.BuildFactory(project=default_project, source=default_source, in_progress=True)
    db_session.add(build)
