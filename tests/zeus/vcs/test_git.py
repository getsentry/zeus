...
def _set_author(name, email):
    check_call('cd {0} && git config --replace-all "user.name" "{1}"'
               .format(remote_path, name), shell=True)
    check_call('cd {0} && git config --replace-all "user.email" "{1}"'
               .format(remote_path, email), shell=True)


@pytest.fixture(scope='function', autouse=True)
def repo_config():
    check_call('rm -rf %s' % (root,), shell=True)
    check_call('mkdir -p %s %s' %
               (path, remote_path), shell=True)
...
def test_log_with_branches(vcs):
    ...
    check_call('cd %s && git checkout %s' % (
        remote_path, vcs.get_default_revision(),
    ), shell=True)
    check_call('cd %s && git checkout -b B3' %
               remote_path, shell=True)
...
def test_get_known_branches(vcs):
    ...
    check_call('cd %s && git checkout -B test_branch' % remote_path,
               shell=True)
    vcs.update()
    branches = vcs.get_known_branches()
