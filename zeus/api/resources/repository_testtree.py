...
class RepositoryTestTreeResource(BaseRepositoryResource):
    def get(self, repo: Repository):
        ...
        job_list = db.session.query(Job.id).filter(Job.build_id == latest_build.id)
        ...
        if parent:
            test_list = test_list.filter(TestCase.name.startswith(parent))
        ...
        if test_list:
            ...
            groups = build_tree(
                [t[0] for t in test_list], sep=sep, min_children=2, parent=parent
            )
            ...
            if parent:
                for chunk in parent.split(sep):
                    ...
                    trail.append({"path": sep.join(context), "name": chunk})
        else:
            results = []
            trail = []
