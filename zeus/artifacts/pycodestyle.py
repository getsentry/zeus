...
regexp = re.compile(
    r"^(?P<filename>[^\:]+)\:(?P<lineno>\d+)\:(?P<colno>\d+)\: (?P<source>[a-z]\d+) (?P<message>.+)$",
    re.I,
)

SEVERITY_MAP = {"W": Severity.warning, "E": Severity.error, "F": Severity.error}
...
class PyCodeStyleHandler(ArtifactHandler):
    ...
    def process(self, fp):
        ...
        for line in fp:
            ...
            severity = SEVERITY_MAP.get(parsed.group("source")[0], Severity.info)
