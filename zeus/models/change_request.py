...
class ChangeRequest(RepositoryBoundMixin, StandardAttributes, db.Model):
    ...
    author_id = db.Column(GUID, db.ForeignKey(
        'author.id'), index=True, nullable=True)
    ...
    head_revision = db.relationship(
        'Revision', foreign_keys='[ChangeRequest.repository_id, ChangeRequest.head_revision_sha]', viewonly=True
    )
    ...
    __table_args__ = (
        ...
        db.ForeignKeyConstraint(
            ('repository_id',
             'head_revision_sha'), ('revision.repository_id', 'revision.sha')
        ),
        db.Index('idx_cr_parent_revision',
